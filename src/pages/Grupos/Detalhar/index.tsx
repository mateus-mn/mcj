...
{
	const { data, isLoading } = useGetDetalharGrupo(
	grupo?.id,
	{
	...
	const { data : historico, isLoading : isLoadingHistorico } = useGetHistoricoGrupo(
		grupo?.id,
		{
			refetchInterval      : false,
			refetchOnWindowFocus : false,
												...
												historico?.map ((item) =>
												{
													return (
														<tr key={item.id}>
															...
															<td> {moment (item.dataHoraRegistro).format ("DD/MM/YYYY")} </td>
															<td> {moment (item.dataHoraRegistro).format ("HH:mm:ss")} </td>
